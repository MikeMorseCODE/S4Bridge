...
1. Visit Home, Capture, Mapping, MIDI, and Settings from the bottom navigation
   on both a compact phone and a larger device; confirm every page scrolls and
   no controls are obscured by system bars.
2. Confirm Home, Capture, and Mapping reflect connection, deck, report-count,
   and semantic-control changes without visible stalls during fast fader sweeps.
3. Confirm continuous controls do not flood the Capture event log and the UI
   refresh rate remains bounded, while button press/release events remain visible.
4. Confirm the unavailable MIDI/export actions are disabled and that the
   MediaPlayer/native-audio limitation is clearly identified in Settings.
5. Disable auto-reconnect in Settings, reconnect the controller, and verify it
   remains idle until **Start Capture** is selected. Re-enable it and repeat the
   automatic reconnect checks below.
