...
The application uses five bottom-navigation destinations:

- **Home** shows live controller connection, library selection, both MediaPlayer decks, and loading controls.
- **Capture** controls and monitors the real USB/HID session, report counters, latest semantic event, and bounded event log.
- **Mapping** presents the preserved known-good mapping as read-only and shows coalesced live input.
- **MIDI** previews the planned routing surface, with output and export actions visibly unavailable because there is no MIDI backend yet.
- **Settings** controls automatic USB reconnect and identifies the fixed controller target. Native audio, DSP, MIDI, and LED output remain clearly identified as future work.

Continuous fader/knob state is coalesced to at most 20 UI refreshes per second. Discrete button events remain visible in the Capture log; continuous reports do not spam it.
